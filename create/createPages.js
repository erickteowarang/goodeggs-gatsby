...
const componentsFolder = path.join(process.cwd(), 'src', 'components', 'organisms')
...
const createTemporaryPageTemplateFile = (
  ...
  const componentImportString = componentNames
    .map(componentName => {
      return `import ${componentName} from '../../src/components/organisms/${componentName}'`
    })
    .join('\n')
...
const getComponentFragments = (
  ...
  componentNames.forEach(componentName => {
    ...
    if (existsSync(componentFragmentFile)) {
      // The fragment file is cached when initially required on `npm run dev`, so if we update
